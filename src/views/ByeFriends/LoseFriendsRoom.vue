<template>
  <GameLayout name-game="Як втратити друзів">
    <div class="containerFormCreate">
      <div class="formCreate" v-if="question">
        <div class="card">
          <h2>{{ question }}</h2>
        </div>
      ...
      <div class="modal-content">
        ...
        <section class="modal-body">
          <div class="winner">
            ...
            <h4>Голоса за:</h4>
            <ul>
              <li v-for="voter in winner.votes" :key="voter">{{ voter }}</li>
            </ul>
          </div>
        </section>
        <footer class="modal-footer">
          <button @click="nextRound" class="btn-grad">Продолжить</button>
        </footer>
      </div>
    </div>
  </GameLayout>
...
const connectWebSocket = () => {
  ...
  ws.onmessage = (event) => {
    ...
    if (data.question) {
      ...
      winner.value = null;
      selectedPlayerId.value = null; // Сбрасываем выбор после завершения голосования
    }
    if (data.players) {
...
.player.selected {
  border: 3px solid #000; /* Обводка для выбранного игрока */
  transform: scale(1.1);
}
